...
public class Group extends AbstractEntity {
    ...
    public Group(String name, String about, String logoImage, User createdBy) {
        ...
        this.univName = createdBy.getUnivInfo().univName();
    ...
    public void addMember(User user, GroupRole role) {
        Assert.isTrue(this.univName.equals(user.getUnivInfo().univName()), "동일한 학교 소속의 유저만 가입할 수 있습니다.");
    ...
    public void addJoinRequest(User user) {
        Assert.state(this.univName.equals(user.getUnivInfo().univName()), "동일한 학교 소속의 유저만 가입을 요청할 수 있습니다.");
        Assert.state(
                this.members.stream()
    ...
    public void closeRecruitment() {
        Assert.notNull(this.recruitmentPost, "모집 공고가 존재하지 않습니다.");
        this.recruitmentPost = null;
    }
}